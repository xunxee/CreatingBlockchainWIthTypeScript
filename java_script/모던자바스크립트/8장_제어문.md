...
- 일반적으로 코드는 위에서 아래 방향으로 순차적으로 실행된다. 그러나 **제어문**으로 **코드의 실행 흐름을 인위적으로 제어**할 수 있다.
- 하지만 코드의 실행 순서가 변경된다는 것은 단순히 위에서 아래로 순차적으로 진행되는 직관적인 흐름을 혼란스럽게 만든다. 따라서 제어문은 코드의 흐름을 이해하기 어렵게 만들어 **가독성을 해치는 단점**이 있다.
- 제어문을 바르게 이해하는 것은 코딩 스킬에 많은 영향을 준다. 특히 `for`**문은 매우 중요하다**.
